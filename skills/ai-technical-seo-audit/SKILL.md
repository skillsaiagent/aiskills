...
# ai-technical-seo-audit SEO/AEO 诊断助手
...
它会结合网站页面与 SEO 线索、可粘贴页面正文、标题标签、导航结构、关键词备注、搜索表现说明或已有…等输入，整理关键上下文，并输出摘要、诊断结论、行动建议和可复用交付物，便于继续执行、复盘或交付。
...
```bash
python3 scripts/run.py --params '{"goal":"审计目标"}'
```
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `goal` | string | 否 | - | 例如提升自然流量、改善 AI 引用、修复内容结构、提高询盘或梳理内容缺口 |
| `audience` | string | 否 | - | 说明搜索人群、采购角色、意图阶段或主要使用场景 |
| `materialUrl` | string | 否 | - | 填写无需登录即可访问的页面、栏目、站点地图或公开案例链接；需要传可访问的完整 URL |
| `materialFile` | string | 否 | - | 支持网页导出、站点清单、关键词表、内容清单、截图说明或审计文档 |
| `materialText` | string | 否 | - | 可粘贴页面正文、标题标签、导航结构、关键词备注、搜索表现说明或已有审计发现 |
| `brandRequirements` | string | 否 | - | 补充品牌语气、必须保留的事实、禁用表达、行业合规限制或内部规范 |
...
- 输入材料解读：结合审计目标（例如提升自然流量、改善 AI 引用、修复内容结构、提高询盘或梳理内容缺口。）、目标搜索用户（说明搜索人群、采购角色、意图阶段或主要使用场景。）、待审页面或站点链接（填写无需登录即可访问的页面、栏目、站点地图或公开案例链接。）提炼关键上下文和判断依据。
