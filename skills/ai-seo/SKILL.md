...
它会结合AI 搜索优化内容、可粘贴页面正文、产品说明、FAQ、知识库条目、实体介绍或需要提升 …等输入，整理关键上下文，并输出摘要、诊断结论、行动建议和可复用交付物，便于继续执行、复盘或交付。
...
```bash
python3 scripts/run.py --params '{"goal":"GEO 优化目标"}'
```
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `goal` | string | 否 | - | 例如提升 AI 摘要可引用性、强化实体识别、改善 FAQ 覆盖或提高线索转化 |
| `audience` | string | 否 | - | 说明用户会向 AI 搜索提出的问题、角色、意图阶段或决策场景 |
| `materialUrl` | string | 否 | - | 填写无需登录即可访问的页面、文章、文档或参考链接；需要传可访问的完整 URL |
| `materialFile` | string | 否 | - | 支持文章草稿、页面文档、知识库文件、产品资料或 FAQ 表 |
| `materialText` | string | 否 | - | 可粘贴页面正文、产品说明、FAQ、知识库条目、实体介绍或需要提升 AI 摘要质量的内容 |
| `brandRequirements` | string | 否 | - | 补充品牌命名、事实边界、必须保留的描述、禁用表达和合规限制 |
...
- 输入材料解读：结合GEO 优化目标（例如提升 AI 摘要可引用性、强化实体识别、改善 FAQ 覆盖或提高线索转化。）、目标提问者（说明用户会向 AI 搜索提出的问题、角色、意图阶段或决策场景。）、AI 搜索目标页面（填写无需登录即可访问的页面、文章、文档或参考链接。）提炼关键上下文和判断依据。
