...
它会结合飞书文档正文、可粘贴飞书文档正文、评论摘录、目录、会议纪要、任务说明或导出的文本…等输入，整理关键上下文，并输出摘要、诊断结论、行动建议和可复用交付物，便于继续执行、复盘或交付。
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `goal` | string | 否 | - | 例如摘要、审阅、改写、行动项整理、风险复盘、对外同步或会议跟进 |
| `audience` | string | 否 | - | 说明这份飞书文档简报面向谁，例如项目组、管理层、客户、评审、协作者或外部伙伴 |
| `materialUrl` | string | 否 | - | 填写用户可公开分享的飞书文档链接；仅作人工链接线索，不代表可读取工作区内容；需要传可访问的完整 URL |
| `materialFile` | string | 否 | - | 上传从飞书手动导出的 docx/pdf/md/txt 文件或其他可解析文档 |
| `materialText` | string | 否 | - | 可粘贴飞书文档正文、评论摘录、目录、会议纪要、任务说明或导出的文本内容 |
| `brandRequirements` | string | 否 | - | 补充组织术语、输出语气、保密边界、必须保留信息和不可外传内容 |
...
- 输入材料解读：结合处理目标（例如摘要、审阅、改写、行动项整理、风险复盘、对外同步或会议跟进。）、同步对象（说明这份飞书文档简报面向谁，例如项目组、管理层、客户、评审、协作者或外部伙伴。）、飞书公开链接（填写用户可公开分享的飞书文档链接；仅作人工链接线索，不代表可读取工作区内容。）提炼关键上下文和判断依据。
