...
它会结合飞书表格数据、可粘贴飞书表格、CSV、字段说明、公式片段、指标口径或导出的文本内…等输入，整理关键上下文，并输出摘要、诊断结论、行动建议和可复用交付物，便于继续执行、复盘或交付。
...
| 参数 | 类型 | 必填 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `goal` | string | 否 | - | 例如数据体检、进度汇总、指标解释、异常排查、名单清洗或对外同步 |
| `audience` | string | 否 | - | 说明这份表格简报面向谁，例如项目组、运营、财务、销售、管理层、数据团队或客户 |
| `materialUrl` | string | 否 | - | 填写用户可公开分享的飞书表格链接；仅作人工链接线索，不代表可读取工作区数据；需要传可访问的完整 URL |
| `materialFile` | string | 否 | - | 上传从飞书手动导出的 xlsx/csv/tsv/pdf 文件或其他可解析表格 |
| `materialText` | string | 否 | - | 可粘贴飞书表格、CSV、字段说明、公式片段、指标口径或导出的文本内容 |
| `brandRequirements` | string | 否 | - | 补充指标口径、字段命名、展示格式、保密边界、必须保留列和对外叙述限制 |
...
- 输入材料解读：结合处理目标（例如数据体检、进度汇总、指标解释、异常排查、名单清洗或对外同步。）、报告读者（说明这份表格简报面向谁，例如项目组、运营、财务、销售、管理层、数据团队或客户。）、飞书表格链接（填写用户可公开分享的飞书表格链接；仅作人工链接线索，不代表可读取工作区数据。）提炼关键上下文和判断依据。
